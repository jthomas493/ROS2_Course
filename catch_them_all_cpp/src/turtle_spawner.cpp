...
public:
    TurtleSpawnerNode() : Node("turtle_spawner"), turtle_counter_(0)
    {
        ...
        catch_turtle_ = this->create_service<my_robot_interfaces::srv::CatchTurtle>("catch_turtle", std::bind(&TurtleSpawnerNode::callbackCatchTurtle, this, std::placeholders::_1, std::placeholders::_2));
    }
...
private:
                             ...
                             const my_robot_interfaces::srv::CatchTurtle::Response::SharedPtr response)
    {
        kill_turtle_threads_.push_back(
            std::make_shared<std::thread>(std::bind(&TurtleSpawnerNode::callTurtleKill, this, request->name)));
    ...
    void spawnTurtle()
    {
        ...
        spawn_turtle_threads_.push_back(
            std::make_shared<std::thread>(
                std::bind(&TurtleSpawnerNode::callTurtleSpawner, this, x, y, theta)));
    }

    void callTurtleSpawner(double x, double y, double theta)
    {
        auto client = this->create_client<turtlesim::srv::Spawn>("spawn");
        while (!client->wait_for_service(std::chrono::seconds(1)))
        {
            RCLCPP_WARN(this->get_logger(), "Wating for turtlesim spawner...");
        }
        ...
        try
        {
            ...
            if (response->name != "")
            {
                auto spawned_turtle = my_robot_interfaces::msg::Turtle();
                ...
                publishAliveTurtles();
                RCLCPP_INFO(this->get_logger(), "Turtle %s has spawned.", response->name.c_str());
            }
        }
    ...
    void callTurtleKill(std::string name)
    {
        auto client = this->create_client<turtlesim::srv::Kill>("kill");
        while (!client->wait_for_service(std::chrono::seconds(1)))
        {
            RCLCPP_WARN(this->get_logger(), "Wating for turtlesim kill server...");
        }
    ...
    rclcpp::Service<my_robot_interfaces::srv::CatchTurtle>::SharedPtr catch_turtle_;
